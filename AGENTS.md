...
- The `codex` provider must call `codex exec` with ephemeral sessions and a read-only sandbox. Use the Codex CLI configured model unless the user passes `--model`.
