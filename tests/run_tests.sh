...
# Reference: real_test.fr.de.srt (generated by auto --mix, verified correct)
...
if [[ -f "$real_mix" ]]; then
    ...
    ref_count=$(grep -c '^[0-9][0-9]*$' "$FIXTURES/real_test.fr.de.srt")
    # Block count should match reference (430)
    if [[ "$real_mix_count" -eq "$ref_count" ]]; then
        assert "real mix: block count matches reference ($ref_count)" 0
    else
        assert "real mix: block count matches reference (got $real_mix_count, expected $ref_count)" 1
    fi

    # First block: DE "Chicken Nuggets" paired with FR "nuggets de poulet"
    ...
    # Block 10: check a mid-file block for correct pairing
    # Extract block 10 from both generated and reference, compare DE text
    gen_b10=$(awk '/^10$/{found=1;next} found && /^$/{exit} found{print}' "$real_mix" | head -1)
    ref_b10=$(awk '/^10$/{found=1;next} found && /^$/{exit} found{print}' "$FIXTURES/real_test.fr.de.srt" | head -1)
    # Skip the timestamp line (line 1), get the text line
    gen_b10_ts=$(awk '/^10$/{found=1;next} found{print;exit}' "$real_mix")
    ref_b10_ts=$(awk '/^10$/{found=1;next} found{print;exit}' "$FIXTURES/real_test.fr.de.srt")
    if [[ "$gen_b10_ts" == "$ref_b10_ts" ]]; then
        assert "real mix: block 10 timestamp matches reference" 0
    else
        assert "real mix: block 10 timestamp matches reference (got '$gen_b10_ts')" 1
    fi

    # Last dialogue block (block 427 in reference): should have both languages
    b427=$(awk '/^427$/{found=1} found{print} found && /^$/{exit}' "$real_mix")
    if echo "$b427" | grep -q "<i>"; then
        assert "real mix: block 427 has italic (both languages)" 0
    else
        assert "real mix: block 427 has italic (both languages)" 1
    fi
